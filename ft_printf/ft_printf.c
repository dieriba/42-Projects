...
int	ft_display_format(const char *format, size_t *i, char c, va_list args_infos)
{
	...
	if (c == 'c')
		...
		count = ft_write_hex_adress((unsigned long int)va_arg(args_infos, unsigned long int), &count);
	else if (c == 'd' || c == 'i')
		count = ft_putnbr((long int)va_arg(args_infos, long int), &count);
	else if (c == 'u')
...
int ft_printf(const char *format, ...)
{
	...
	while ((char)format[i])
	{
		if ((char)format[i] != '%')
			...
			count += ft_display_format(format, &i, (char)format[++i], args_infos);
		i++;
	}
	va_end(args_infos);
	return (count);
